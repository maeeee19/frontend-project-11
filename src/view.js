...
const pushPostDOM = (post) => {
  ...
  postItem.innerHTML = `
        <a class="fw-bold" href="${post.link}" data-id="527" target="_blank" rel="noopener noreferrer">
            ${post.title}
        </a>
        <button type="button" class="btn btn-outline-primary btn-sm" onclick="openModal(${JSON.stringify(post)})">Просмотр</button>
    `;
...
const watchState = (object) => onChange(object, (path, value, previousValue, applyData) => {
  ...
  if (path === 'feeds') {
    const foundIndexFeed = previousValue.findIndex(
      (item) => item.link === applyData.args[0].link,
    );
    ...
    if (foundIndexFeed === -1) {
      ...
      const filteredPostsIds = newPostsIds.filter(
        (item) => !oldPostsIds.includes(item),
      );
      const filteredPosts = filteredPostsIds.map(
        (item) => newPosts.find((newPost) => newPost.id === item),
      );

      filteredPosts.forEach((item) => pushPostDOM(item));
    }
  }
});
